...
from Functions.Supporting_Functions import (stdBm_incremental, stdBm_cholesky, stdBm_bridge, 
                                            convergenceTestBm)

#%% Process

def run(latex_mode, quasiMC, nPathsPower, nSteps, nFactors, horizon, x0, methods):
    ...
    dfExp, dfVar = convergenceTestBm(quasiMC, nPathsPower, nSteps, nFactors, horizon, x0, methods)
...
if __name__ == "__main__":
    ...
    quasiMC     = True
    latex_mode  = True
    methods     = ["loop", "matrix", "bridgeSobol", "PCA", "Spectral"]    
    nPathsPower = [2, 3, 4, 5, 6, 7, 8, 9, 10] #nPaths = 2**nPathsPower    
    nSteps      = 2**9 #should be power two for Bridge and Sobol 
    horizon     = 1.0
    x0          = 0.0
    # nPaths      = 3
    nFactors    = 10 #For PCA. Effecively reducing dimension from nSteps to nFactors
    
    # Convergence -------------------------------------------------------------
    dfExp, dfVar = run(latex_mode, quasiMC, nPathsPower, nSteps, nFactors, horizon, x0, methods)
    ...
    # Sim ---------------------------------------------------------------------
    # sim_loop   = stdBm_incremental(nPaths, nSteps, horizon, x0)    
    # sim_mat    = stdBm_cholesky(nPaths, nSteps, horizon, x0) 
